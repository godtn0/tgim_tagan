...
parser.add_argument('--trainclasses_file', type=str, 
                    help='text file that contains training classes')
parser.add_argument('--save_filename_G', type=str, 
                    help='checkpoint file of generator')
parser.add_argument('--save_filename_D', type=str, 
...
if __name__ == '__main__':
    ...
    train_data = TextDataset(cfg.data_dir, transform=image_transform )
    ...
    G = Generator(train_data.n_words)
    D = Discriminator(train_data.n_words)
    damsm = DAMSM(train_data.n_words, cfg.w_dim, cfg.embedding_dim, cfg.ch)
    G, D, damsm = G.to(device), D.to(device), damsm.to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        while step < num_batches:
            ...
            word_region_sim = damsm(img, txt, len_txt)
            word_region_loss = F.binary_cross_entropy_with_logits(word_region_sim, ones_like(word_region_sim))
            word_region_loss.backward()
